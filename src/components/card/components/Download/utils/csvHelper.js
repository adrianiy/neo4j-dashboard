...
const csvChain = (input) => (Array.isArray(input) ? input : []).map(serializeObject).map(csvEscape).join(csvDelimiter);
...
export const CSVSerializer = (cols) => {
    ...
    return {
        ...
        output: () => csvChain(_cols) + (!_data.length ? "" : csvNewline + _data.map(csvChain).join(csvNewline)),
    };
};
